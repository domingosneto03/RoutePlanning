//
// Created by domin on 19/03/2025.
//

#ifndef DA_PROJECT1_READER_H
#define DA_PROJECT1_READER_H
...
template<class T>
void Reader<T>::loadLocations(Graph<T>& graph, const string& filename) {
...
template <class T>
void Reader<T>::loadDistances(Graph<T>& graph, const string& filename) {
    ifstream file(filename);
    if (!file.is_open()) {
        cerr << "Error opening file: " << filename << endl;
        return;
